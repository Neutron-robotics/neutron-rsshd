...
if [ -z "$ROOT_PASSWORD" ]; then
    ROOT_PASSWORD=$(< /dev/urandom tr -dc A-Z-a-z-0-9 | head -c8)
    echo "==========="
...
# Create neutron-user with no special privileges
if ! id -u neutron-user > /dev/null 2>&1; then
    adduser -D -s /sbin/nologin neutron-user
    
    if [ -z "$NEUTRON_PASSWORD" ]; then
        NEUTRON_PASSWORD=$(< /dev/urandom tr -dc A-Z-a-z-0-9 | head -c8)
        echo "==========="
        echo "== Neutron-user Password is $NEUTRON_PASSWORD"
        echo "==========="
        echo
    fi
    
    echo "neutron-user:${NEUTRON_PASSWORD}" | chpasswd
fi
...
# Configure neutron-user for reverse SSH tunnel
cat << EOF >> $SDIR/sshd_config

# Configuration for neutron-user
Match User neutron-user
    PermitTunnel yes
    PermitOpen any
    PermitRootLogin no
